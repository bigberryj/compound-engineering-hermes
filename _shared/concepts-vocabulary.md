...
`CONCEPTS.md` defines the words that mean something specific in this codebase — substrate that `docs/solutions/` and `AGENTS.md` can cite without redefinition. Lives at the project repo root. Terms enter two ways — accretion and seeding (below) — and the file is created the first time either path produces a qualifying entry.
...
Each entry teaches its concept to a reader with no access to anything else — no codebase, no PR history, no architecture meetings, no chat. This rules out:
