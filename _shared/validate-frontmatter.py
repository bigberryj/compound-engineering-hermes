...
Ported to Hermes from EveryInc/compound-engineering-plugin
(ce-compound/scripts/validate-frontmatter.py), unchanged behavior.
"""
import os
import re
import sys
...
def main(argv):
    ...
    for lineno, line in enumerate(fm_text.split("\n"), start=2):
        ...
        if line.startswith((" ", "\t")):
            continue
        # Skip pure list-marker lines like "- item" (defensive)
        ...
        if not val_stripped:
            ...
            # block (e.g. `tags:` followed by `- foo`). Nothing to validate.
            continue
