...
1. **Ask, don't prescribe.** Do not offer menu options for open answers (problem, approach, persona). Use `clarify` in open-ended mode. Reserve multi-choice for routing decisions.
...
Once sections 1–5 are captured (and any optional sections the user engaged with), read `strategy-template.md` and fill it in. Present the full draft in chat before writing. Offer one edit round. Then write to `STRATEGY.md` at the project root.
