...
argument-hint: "[lookback window, e.g. '24h', '7d', '1h'; default 24h] [setup|reconfigure]"
version: 1.0.0
...
metadata:
  hermes:
    tags: [analytics, observability, metrics, pulse, read-only]
    related_skills: [compound-engineering, ce-strategy, ce-compound, ce-setup]
---

# Product Pulse (`ce-product-pulse`)

Query the product's data sources for a given time window and produce a compact, single-page report covering usage, performance, errors, and followups. The report is saved to `docs/pulse-reports/` and the key points are surfaced in chat.

The skill does not mutate the product, the database, or any external system. Its only writes are pulse settings appended to `.compound-engineering/config.local.yaml` (the unified CE local config, gitignored, machine-local) and the report file (`docs/pulse-reports/...`). Read-only data-source integrations (analytics, tracing, payments, DB) are invoked read-only; if a tool offers write modes, do not use them.

## Interaction Method

Use `clarify` for any user-facing questions. Ask one question at a time. Reserve multi-select for first-run configuration only.
...
If the argument is empty, default to `pulse_lookback_default` from config (resolved in Phase 0); if that is also unset, fall through to the hard default of `24h`. If the argument is unparseable, ask the user to clarify. The argument `setup` or `reconfigure` routes to Phase 1 regardless of any other tokens.
...
Upstream `ce-product-pulse` assumes MCP servers for PostHog, Sentry, Stripe, and read-only DB are configured. In Hermes these MCP integrations are not pre-installed. **The skill still works** — it gracefully degrades to whatever data sources the user has configured in `.compound-engineering/config.local.yaml`. If no `pulse_analytics_source` is set, the analytics section of the report shows `not configured` instead of numbers. The user can wire up data sources over time (custom MCP servers, external CLIs, or pre-fetched JSON dumps referenced from config).

The bundled config template (`.compound-engineering/config.local.example.yaml`) documents the keys; the user fills in only what they have.

## Execution Flow

### Phase 0: Route by Config State

Resolve the project root with `git rev-parse --show-toplevel` (fall back to `pwd`). Read `<project-root>/.compound-engineering/config.local.yaml`. If the root cannot be resolved or the file does not exist, treat this as a first run. Otherwise extract values for the `pulse_*` keys listed under "Config keys" below.
...
- `pulse_quality_dimension` — string scored 1-5 when `pulse_quality_scoring` is true
...
- `pulse_metric_sources` — comma-separated `metric=source` pairs giving per-strategy-metric source overrides
- `pulse_pending_metrics` — comma-separated string of strategy-doc metric names awaiting instrumentation; rendered as `no data` until instrumentation lands
- `pulse_excluded_metrics` — comma-separated string of strategy-doc metric names intentionally excluded from the pulse
...
Before asking any questions, read `STRATEGY.md` at the project root. If the file exists, extract:

- The product name from the `name` key in the YAML frontmatter, falling back to the H1 title (stripping the trailing ` Strategy` suffix) if frontmatter is missing
...
Read `references/interview.md` (load with `read_file ~/.hermes/skills/compound-engineering/ce-product-pulse/references/interview.md`). This load is non-optional — the pushback rules, anti-pattern examples, and metric-to-source mapping logic live there.
...
Write the captured config to `<project-root>/.compound-engineering/config.local.yaml` as flat `pulse_*` keys, using the schema in `references/interview.md` under "Config file shape". Resolve the project root with `git rev-parse --show-toplevel` (or `pwd`). To write: (1) if the file or directory does not exist, create `.compound-engineering/` and write the YAML file; (2) if the file exists, merge new keys into the existing YAML, preserving any non-pulse keys (e.g., `work_delegate_*`) untouched. If `.compound-engineering/config.local.yaml` is not already covered by the repo's `.gitignore`, offer to add the entry before writing. Show the resulting pulse block to the user in chat and offer one round of edits.

After the config is written, run the **scheduling recommendation** from `references/interview.md` section 9: offer to set up a recurring run so the user gets the pulse on a cadence instead of having to remember to run it. Accept yes/no/later. If yes, hand off to Hermes's cron primitive (`cronjob` action='create' with a schedule like `0 9 * * *` for daily at 9am). Do not schedule inline. Then proceed to Phase 2.

### Phase 2: Run the Pulse

If Phase 1 ran (first run, or `setup`/`reconfigure` argument), re-read `.compound-engineering/config.local.yaml` from the project root to pick up any edits accepted during the Phase 1 review step. Otherwise, use the `pulse_*` values already extracted in Phase 0. Apply hard defaults for any unset settings (see Phase 0 "Config keys").

#### 2.1 Dispatch Queries

Run these in **parallel** (different tools, no shared load) — only for sources the user has actually configured:
...
**Hermes fallback:** when a source is configured but no MCP server or pre-fetched data is available, the section renders `not configured at runtime — see .compound-engineering/config.local.yaml` and the user can wire it up later. The skill does not fail.
...
**No PII in the score summary.** Capture a count distribution (e.g., "8x 5, 1x 4, 1x 2") and a short anonymized note on any session scored below 4.

#### 2.3 Assemble the Report

Read `references/report-template.md` (load with `read_file ~/.hermes/skills/compound-engineering/ce-product-pulse/references/report-template.md`). Fill in the template using the query results. Four sections, in order:
...
- If the argument was a known schedule keyword (`daily`, `hourly`, `weekly`), note that this run is ad-hoc and suggest scheduling via Hermes's `cronjob` tool for recurring runs.
