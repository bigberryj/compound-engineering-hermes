...
description: "Refresh stale learning and pattern docs under docs/solutions/ by reviewing them against the current codebase, then updating, consolidating, replacing, or deleting drifted ones. Use when the user asks to 'refresh my learnings', 'audit docs/solutions/', 'clean up stale learnings', or 'consolidate overlapping docs', or when ce-compound flags an older doc as superseded. Do not trigger for general refactor, debugging, or code-review work unless the user has explicitly pointed at docs/solutions/."
...
metadata:
  hermes:
    tags: [maintenance, knowledge, documentation, refresh, audit, consolidation]
    related_skills: [compound-engineering, ce-compound, ce-strategy, systematic-debugging, requesting-code-review]
---
...
Check arguments for `mode:headless`. If present, strip it from arguments (use the remainder as a scope hint) and run in **headless mode**.
...
- **Mark as stale when uncertain.** If classification is genuinely ambiguous (Update vs Replace vs Consolidate vs Delete) or Replace evidence is insufficient, mark as stale with `status: stale`, `stale_reason`, and `stale_date` in the frontmatter.
...
- Ask questions **one at a time** using `clarify`
...
3. **Match docs to reality, not the reverse.** When current code differs from a learning, update the learning to reflect the current code. The skill's job is doc accuracy, not code review — do not ask the user whether code changes were "intentional" or "a regression." If the code changed, the doc should match.
4. **Be decisive, minimize questions.** When evidence is clear (file renamed, class moved, reference broken), apply the update. In interactive mode, only ask the user when the right action is genuinely ambiguous. In headless mode, mark ambiguous cases as stale instead.
...
9. **Evaluate document-set design, not just accuracy.** In addition to checking whether each doc is accurate, evaluate whether it is still the right unit of knowledge. If two or more docs overlap heavily, determine whether they should remain separate, be cross-scoped more clearly, or be consolidated into one canonical document.
10. **Delete, don't archive.** There is no `_archived/` directory. When a doc is no longer useful, delete it. Git history preserves every deleted file.
...
If arguments are provided, use them to narrow scope before proceeding. Try these matching strategies in order, stopping at the first that produces results:
...
If no matches are found, report that and ask the user to clarify. In headless mode, when a scope hint was provided but matched nothing, report the miss in the summary and exit without widening to all docs — do not silently fall back to processing everything.
...
- **Memory** — does the system-prompt memory block contain entries in the same problem domain? Scan it directly. A memory note describing a different approach than what the learning recommends is a supplementary drift signal.
...
- Add context to the evidence report ("(agent memory) notes suggest approach X may have changed since this learning was written")
...
Use subagents (via `delegate_task`) for context isolation when investigating multiple artifacts. Choose the lightest approach that fits:
...
> Use dedicated file search and read tools for all investigation. Do NOT use shell commands (ls, find, cat, grep, test, bash) for file operations. This avoids permission prompts and is more reliable.
>
> Also scan the system-prompt memory block (Hermes's MEMORY/USER PROFILE sections). Check for notes related to the learning's problem domain. Report any memory-sourced drift signals separately from codebase-sourced evidence, tagged with "(agent memory)" in the evidence section. If the block is not present in your context, skip this check.
...
The learning is still accurate and useful. Do not edit the file — report that it was reviewed and remains trustworthy. Only add `last_updated` if you are already making a meaningful update for another reason.
...
- Prefer the platform's content-search tool (`search_files target=content` in Hermes) over shell.
...
**Inbound links inform the classification, not the cleanup.** Removing a citation is always mechanical. The judgment is upstream: given these citations, is Delete still right, or is Replace closer to right?
...
In headless mode, Delete + decorative cleanup is fine. Any substantive citation, or any genuine ambiguity, downgrades to stale-marking.
...
- You are about to Delete a document **and** the evidence is not unambiguous. When auto-delete criteria are met, proceed without asking.
...
Always present choices using `clarify`. One question at a time. Lead with the recommended option. Explain the rationale for the recommendation in one concise sentence. Avoid asking the user to choose from actions that are not actually plausible.
...
Then ask via `clarify`. Do not list all five actions unless all five are genuinely plausible.
...
- **Update** — in-place edits when the solution is still substantively correct.
...
**First, read `references/concepts-vocabulary.md`.** This is unconditional. Do not pre-judge from memory which Phase 1 signals qualify.
...
6. **Scrub violations.** Scan existing entries for content that violates `references/concepts-vocabulary.md` criteria — implementation specifics, current-config values, status/owner/date metadata, duplicates, or entries that lean on an undefined project-specific sibling.

If no Phase 1 signals qualified, record that outcome explicitly in the report's `CONCEPTS.md` line.
...
- What evidence was found — tag any memory-sourced findings with "(agent memory)" to distinguish them from codebase-sourced evidence
...
In headless mode, the report is the sole deliverable. **Print the full report. Do not abbreviate, summarize, or skip sections.**
...
If all writes succeed, the Recommended section is empty. If no writes succeed (e.g., read-only invocation), all actions appear under Recommended.
...
First, run `git branch --show-current` to determine the current branch. Then present the correct options based on the result via `clarify`. Stage only compound-refresh files regardless of which option the user picks.

**If the current branch is main, master, or the repo's default branch:**

1. Create a branch, commit, and open a PR (recommended) — the branch name should be specific to what was refreshed
...
**If the current branch is a feature branch, dirty working tree:**

1. Commit only the compound-refresh changes to `{current branch name}` (selective staging)
2. Don't commit
...
Use **Consolidate** proactively when the document set has grown organically and redundancy has crept in. Periodic consolidation keeps the document set lean and authoritative.
...
1. Identify which root-level instruction files exist (AGENTS.md, CLAUDE.md, or both). Read the file(s) and determine which holds the substantive content. The substantive file is the assessment and edit target; ignore shims. If neither file exists, skip this check entirely.
...
4. If not:
   a. Identify where a mention fits naturally. A line added to an existing section is almost always better than a new headed section.
   b. Draft the smallest addition that communicates the three things. Match the file's existing style and density.
   c. In interactive mode, explain to the user why this matters, show the proposed change, then use `clarify` to get consent before making the edit. In headless mode, include it as a "Discoverability recommendation" line in the report — do not attempt to edit instruction files (headless scope is doc maintenance, not project config).
5. **If `CONCEPTS.md` exists at repo root, run a parallel discoverability check for it.** Use the same workflow as the `docs/solutions/` check above.
   **Skip this step entirely if `CONCEPTS.md` does not exist**.
6. **Amend or create a follow-up commit when the check produces edits.** Stage the newly edited file and either amend the existing commit (if still on the same branch and no push has occurred) or create a small follow-up commit.
