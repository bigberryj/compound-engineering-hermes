# Per-Action Flows for `ce-compound-refresh`
...
**File paths in Hermes** (resolved absolutely; not the upstream `${CLAUDE_SKILL_DIR}` env var):

- `~/.hermes/skills/compound-engineering/ce-compound/references/schema.yaml`
- `~/.hermes/skills/compound-engineering/ce-compound/references/yaml-schema.md`
- `~/.hermes/skills/compound-engineering/ce-compound/assets/resolution-template.md`

Do not let replacement subagents invent frontmatter fields, enum values, or section order from memory.

**When evidence is sufficient:**

1. Spawn a single subagent via `delegate_task` to write the replacement learning. Pass it:
...
2. The subagent writes the new learning using the support files as the source of truth: `schema.yaml` for frontmatter fields and enum values, `yaml-schema.md` for category mapping and YAML-safety rules for array items, and `resolution-template.md` for section order. It should use dedicated file search and read tools if it needs additional context beyond what was passed.
3. **Validate parser-safety of the new learning's frontmatter** to catch silent-corruption issues the prose rules miss:

   ```bash
   python3 ~/.hermes/skills/compound-engineering/ce-compound/scripts/validate-frontmatter.py <new-learning-path>
   ```

   - **If the script ran:** exit 0 means parser-safe; exit 1 means stderr names the offending field(s) — quote the value(s), re-write the doc, and re-run until exit 0.
   - **If the script did not run** (else branch, e.g. Python missing): apply the validator's checks by hand, matching its exact scope — checking more broadly risks edits the validator would not require. Fix any violation by quoting the whole value before continuing:
     ...
     Nested values, array items, and already-quoted values are out of scope here.

4. After the subagent completes, the orchestrator deletes the old learning file. The new learning's frontmatter may include `supersedes: [old learning filename]` for traceability, but this is optional.
...
Delete only when a learning is clearly obsolete, redundant (with no unique content to save), or its problem domain is gone. Do not delete a document just because it is old — age alone is not a signal.

Before unlinking the file, run a final inbound-link check across the repo's markdown content to catch any references missed during Phase 1 investigation. Prefer the platform's content-search tool (`search_files target=content` in Hermes) for efficiency; use ranged or context-line reads around matches rather than loading whole files.
